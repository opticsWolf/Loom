...
//! * Data inside a frame is stored as a `SpectralData` view — an `Arc<[f64]>`
//!   plus an offset/length. Cloning is O(1), and unweaving copies the input
//!   curve once and hands each frame a zero-copy window into it.
...
// slow under that pattern — large blocks get committed/decommitted to the OS on
// every alloc/free, and concurrent allocation contends on a global lock.
// mimalloc caches freed blocks, avoids the OS round-trips, and scales better,
// which is what closes the gap with NumPy's allocator on copy-heavy operations.
#[global_allocator]
...
/// Fixed-width fingerprint of a wavelength grid.
///
/// Previously this was a `Vec<u8>` copy of the whole grid (8·n bytes allocated,
/// hashed, and compared on every `set_data`/`unweave`). We now collapse the grid
/// to a 128-bit xxh3 hash: O(1) to store, hash, and compare. A 128-bit space makes
/// accidental collisions astronomically unlikely, and the two call sites that key
/// real data on this (`get_or_create_frame`, `resolve_plan`) additionally verify a
/// bit-exact grid match on a hash hit, so correctness never depends on the hash
...
/// Bit-exact equality of two wavelength grids (matches the original
/// byte-signature semantics: distinguishes -0.0 from 0.0, compares NaN bit
/// patterns). Compiles down to a length check plus a `memcmp`.
#[inline]
...
impl OpticalKey {
    /// Convert back to an owned-`String` tuple for the Python boundary.
    /// Off the hot path (used only when listing keys / building output dicts).
    #[inline]
...
// ---------------------------------------------------------------------------
// SpectralData — Arc-backed handle to a frame's stored curve
// ---------------------------------------------------------------------------
/// A `(buf, start, len)` window over an `Arc<[f64]>`; cloning is an O(1)
/// refcount bump. Today every instance is full-span (`start == 0`), i.e. each
/// frame owns its own buffer — that allocation pattern measured fastest under
/// mimalloc. The `start`/`len` fields keep the door open to zero-copy windowing
/// into a shared buffer later, should profiling favour it.
#[derive(Clone)]
struct SpectralData {
    buf: Arc<[f64]>,
    start: usize,
...
impl SpectralData {
    /// Wrap an owned buffer as a full-span view.
    #[inline]
...
pub struct SpectralDataFrame {
    pub uid: usize,
    data: RwLock<HashMap<OpticalKey, SpectralData>>,
    wavelength: Arc<[f64]>,
    wl_min: f64,
    wl_max: f64,
}
...
impl SpectralDataFrame {
    ...
    fn get_data(&self, key: &OpticalKey) -> Option<SpectralData> {
...
pub struct OpticalCollection {
    ...
    wl_fingerprints: RwLock<HashMap<WlSig, Arc<SpectralDataFrame>>>,
...
impl OpticalCollection {
    ...
    pub fn set_data(
        ...
        // `get_or_create_frame` already guaranteed this frame's grid matches
        // `base_wl` bit-for-bit, so re-validating here would be redundant O(n)
        // work. The length check inside `set_data` still guards correctness.
    ...
    fn get_or_create_frame(&self, wl_arr: &[f64]) -> PyResult<(Arc<SpectralDataFrame>, bool)> {
        let sig = wl_signature(wl_arr);
        {
            ...
            if let Some(frm) = fp.get(&sig) {
                // Confirm the grid actually matches bit-for-bit; only then reuse the
                // frame. Guards against a hash collision silently aliasing two grids.
...
impl SliceOrIndices {
    /// Copy this frame's subset out of the full curve into its own buffer.
    ///
    /// We deliberately copy per frame rather than sharing one big buffer across
    /// frames: smaller buffers stay in the allocator's fast size classes and get
    /// recycled, whereas one large buffer per curve thrashes the OS page mapper.
    #[inline]
    fn gather(&self, data: &[f64]) -> SpectralData {
        match self {
            SliceOrIndices::Slice(s, e) => SpectralData::from_arc(Arc::from(&data[*s..*e])),
...
impl OpticalWeaver {
    ...
    pub fn get_weaved(&self, key: &OpticalKey) -> PyResult<(Vec<f64>, Vec<f64>)> {
        ...
        let mut fragments: Vec<(f64, &[f64], SpectralData)> = Vec::with_capacity(frames.len());
        for frm in &frames {
    ...
    pub fn get_weaved_collections(&self) -> Vec<(Vec<f64>, HashMap<OpticalKey, Vec<f64>>)> {
        let mut groups: HashMap<WlSig, (Vec<f64>, HashMap<OpticalKey, Vec<f64>>)> =
            HashMap::new();
    ...
    fn resolve_plan(&self, full_wavelength: &[f64]) -> PyResult<DistributionPlan> {
        ...
        let current_gen = self.generation.load(Ordering::SeqCst);
        {
            ...
            if let Some((cached_gen, cached_wl, plan)) = cache.get(&sig) {
                // Generation guards staleness; the bit-exact grid check guards
                // against a 128-bit hash collision returning the wrong plan
                // (which would mis-gather data). The compare is a memcmp, no alloc.
        ...
        self.distribution_cache
            .write()
            .put(sig, (current_gen, grid, plan.clone()));
        Ok(plan)
    }
