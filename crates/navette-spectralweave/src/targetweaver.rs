use crate::opticalweaver::{wl_bits_eq, OpticalKey, OpticalWeaver, PyOpticalWeaver, SpectralData, SpectralDataFrame};
...
impl TargetWeaver {
    ...
    fn create_dedicated_frame(&self, wl: &[f64]) -> PyResult<Arc<SpectralDataFrame>> {
    ...
    fn register_metadata(&self, uid: usize, key: OpticalKey, raw_targets: &[f64], tolerances: &[f64], kind: TargetKind, mode_str: &str) {
...
impl PyTargetWeaver {
    ...
    fn add_spectral_target(
        ...
        py.detach(move || -> PyResult<()> {
            ...
            let frame = self.inner.create_dedicated_frame(wl_data)?;
            let data_arc = SpectralData::from_arc(Arc::from(val_data));
            frame.set_data(key.clone(), data_arc, Some(wl_data))?;
    ...
    fn add_angular_target(
        ...
        py.detach(move || -> PyResult<()> {
            ...
            let frame = self.inner.create_dedicated_frame(&wl_point)?;
