...
use crate::providers::{MaterialProvider, grids_equal};
...
impl WovenBackend for navette_spectralweave::opticalweaver::OpticalWeaver {
  fn weaved(&self, prefix: f64, label: &str, material: &str) -> Option<(Vec<f64>, Vec<f64>)> {
    let p = norm_prefix(prefix)?;
    let key = navette_spectralweave::opticalweaver::OpticalKey {
      wavelength: p,
...
/// [`navette_spectralweave::opticalweaver::OpticalWeaver`].
...
impl<B: WovenBackend> WeaverProvider<B> {
  ...
  fn fetch(&self, label: &str, material: &str) -> Result<Option<Vec<f64>>, String> {
    ...
    let spline = navette_interpolate::UniInterpolator::new(
      x,
      y,
      false,
      &self.interp.method,
...
mod tests {
  ...
  fn real_weaver_backend_drives_expansion() {
    use navette_spectralweave::opticalweaver::{OpticalKey, OpticalWeaver, Unit};
    use crate::{ExpandOptions, Layer, expand};
    use std::collections::HashMap;
    ...
    let groups: HashMap<String, crate::Group> = HashMap::new();
