...
use crate::enums::{BlockKind, LayerType};
    ...
    for shared in self.unique_structures() {
      ...
      for (local, layer) in borrowed.layers.iter().enumerate() {
        ...
        match merged.get(layer.material.as_str()) {
          Some(g) if g.optimization_mask[crate::enums::OptMask::Thickness as usize] == 0 => continue,
          _ => {}
        }
        out.push((bi, local));
      }
  ...
  pub fn from_state(value: &Value) -> Result<Self, String> {
    ...
    for (bi, bs) in blocks.iter().enumerate() {
      ...
      let kind = bs
        .get("kind")
        .and_then(|v| v.as_i64())
        .map(|v| crate::enums::BlockKind::try_from_i32(v as i32))
        .transpose()?
        .unwrap_or(crate::enums::BlockKind::Stack);
      arch.add_shared(
        structs[rf].clone(),
...
mod tests {
  ...
  fn merge_conflict_refuses() {
    let mut s1 = flat();
    s1.groups.insert("g".to_string(), crate::shared_group(Group::new("g")));
    let mut s2 = flat();
    let mut g2 = Group::new("g");
    g2.thick_factor = 2.0;
    s2.groups.insert("g".to_string(), crate::shared_group(g2));
    let mut a = Architect::new();
