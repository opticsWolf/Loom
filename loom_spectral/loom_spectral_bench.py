...
Runs an identical workload against:
  ...
  * the Rust extension          (loom_rs, built via `maturin develop --release`)
...
If `loom_rs` is not importable, the script still runs (Python-only): correctness
becomes a self-consistency check and the benchmark reports Python timings alone.

Usage:
    python loom_advanced.py                # both phases
    python loom_advanced.py --bench-only
    python loom_advanced.py --quick        # smaller benchmark sizes
"""
...
try:
    import loom_spectral  # Rust extension (optional)
    HAVE_RUST = True
...
def make_weaver(engine: str, cache_size: int = 128):
    ...
    return loom_spectral.PyOpticalWeaver(cache_size=cache_size)
...
def run_benchmark(quick: bool) -> None:
    ...
    print("=" * 72)
    if not HAVE_RUST:
        print(f"  (loom_rs unavailable: {_RUST_IMPORT_ERR}); Python-only timings.\n")
