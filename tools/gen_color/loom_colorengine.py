...
#
# These are derived from the CIE 1931 2-degree standard observer chromaticities
# the same way colour-science does it: XYZ = xy_to_XYZ(xy), with
#   D65 xy = (0.31270, 0.32900)   ->  [0.9504559270516716, 1, 1.0890577507598784]
#   D50 xy = (0.34570, 0.35850)   ->  [0.9642956764295677, 1, 0.8251046025104605]
# The older rounded ASTM E308 values ([0.95047, 1, 1.08883] / [0.96422, 1, 0.82521])
# differ from colour-science by ~2e-4 in XYZ, which propagates to ~1e-2 errors in
# white-point-dependent spaces (Lab, Luv, UVW). Using the exact xy-derived values
# makes the engine bit-for-bit consistent with colour-science (Lab is exact; Luv,
# UVW and Bradford match to ~1e-13). If navette uses a different D65/D50, change
# ONLY these two lines to navette's constants and regenerate.
# D65: Average daylight (approx 6500K)
REF_WHITE_D65: Final[ArrayFloat] = np.array(
    [0.9504559270516716, 1.0, 1.0890577507598784], dtype=np.float64)
# D50: Horizon daylight (approx 5000K), standard for printing (ICC)
REF_WHITE_D50: Final[ArrayFloat] = np.array(
    [0.9642956764295677, 1.0, 0.8251046025104605], dtype=np.float64)
...
# colour-science verbatim (IEC 61966-2-1 stored matrices). The inverse is
# separately rounded, so it is NOT the exact inverse of the forward matrix
# (round-trip ~3.5e-5); this matches the reference and is intentional.
...
def _din99_kernel(lab1: ArrayFloat, lab2: ArrayFloat, kE: float, kCH: float) -> ArrayFloat:
    ...
    for i in range(n):
        ...
        L99_1 = 105.509 * np.log1p(0.0158 * L1) * kE
        ...
        if G1 < 1e-12:
            ...
            C99_1 = np.log1p(0.045 * G1) / (0.045 * kCH * kE)
        ...
        L99_2 = 105.509 * np.log1p(0.0158 * L2) * kE
        ...
        if G2 < 1e-12:
            ...
            C99_2 = np.log1p(0.045 * G2) / (0.045 * kCH * kE)
...
class ColorSpaceEngine:
    ...
    def _xyz_to_xyY_raw(xyz_array: ArrayFloat) -> ArrayFloat:
        ...
        # Zero-luminance (black) pixels map to (0, 0, 0), matching
        # colour-science's XYZ_to_xyY. (Earlier versions substituted the D65
        # chromaticity, which diverged from the reference at black.)
        xyY[~mask, 0] = 0.0
        xyY[~mask, 1] = 0.0
        xyY[~mask, 2] = 0.0
        return xyY
